...
if [[ -n "$SCRATCH_PATH" ]]; then
  swift build -c "$CONFIGURATION" --scratch-path "$SCRATCH_PATH" --product "$BINARY_NAME"
  swift build -c "$CONFIGURATION" --scratch-path "$SCRATCH_PATH" --product "$CLI_BINARY_NAME"
  BUILD_DIR="$SCRATCH_PATH/$(uname -m)-apple-macosx/$CONFIGURATION"
else
  swift build -c "$CONFIGURATION" --product "$BINARY_NAME"
  swift build -c "$CONFIGURATION" --product "$CLI_BINARY_NAME"
